...
half GetLightAtten(float3 worldPos, float4 pos, half4 dir) {

	half a = saturate(dot(dir.xyz, normalize(worldPos - pos)));
	float d = distance(pos.xyz, worldPos);

	#ifdef UD_SRP
	half dk = pow(1 / d, 2) * (1 - saturate(d / pos.w));
	half ak = pow(1 - saturate((1 - a) / dir.w), 1);
	#else	
	float3 tolight = worldPos - pos.xyz;
	float w = pow(1 / pos.w, 2.5);
	float x = dot(tolight, tolight) * w;
	half dk = tex2Dlod(_LightTextureB0, float4(x, 0.5, 0, 0)).UNITY_ATTEN_CHANNEL;
	half ak = pow(1 - saturate((1 - a) / dir.w), 0.5);
	#endif

	ak *= lerp(ak, 1, 1 - step(0, dir.w));
...
Fragment PrepareFragment(v2f i) {
	...
	#ifdef _EMISSION
	half3 emission = tex2DAutoColor(_EmissionTex, r.uv, r.dd).rgb * _EmissionColor.rgb * _EmissionColor.a;
	r.emission = emission;
...
half3 ForwardLighting(uint instanceID, Fragment f) {
	...
	float4 shadowCoord = mul(unity_WorldToShadow[0], float4(f.worldPos, 1));

	#ifdef UD_SRP
	half shadow = tex2D(_ScreenSpaceShadowmapTexture, f.screenCoord);
	#else
	half shadow = unitySampleShadow(shadowCoord);
	#endif
	...
	float4 pos = UD_LightPos[0];
	float4 dir = UD_LightDir[0];

	light.dir = normalize(pos.xyz - f.worldPos);
	light.color.rgb = UD_LightColor[index].rgb * GetLightAtten(f.worldPos, pos, dir);
